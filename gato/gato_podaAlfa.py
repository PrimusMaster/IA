...
def dibujarTabla(EstadoTabla, eleccionComputadora, eleccionHumano):
    """
    Esta funcion se encarga de imprimir la tabla, recibe la tabla que imprimira y el simbolo 
    que le corresponde a cada usuario
    """
...
def turnoHumano(eleccionComputadora, eleccionHumano):
    """
    Esta funcion se encarga de escribir en el tablero la opcion que 
    se escoge por parte del jugador, las dos variables que recibe
    son los elementos con los que se representan al jugador y a la computadora
    """
    profundidad = len(posicionesVacias(tabla))
    if profundidad == 0 or game_over(tabla):
        return
    ...
    while movimiento < 1 or movimiento > 9:
        try:
            ...
            print('Adios tonoto, te ira mejor en el bote')
            exit()
